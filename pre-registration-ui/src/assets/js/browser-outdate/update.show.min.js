...
var $buo_show = function () {
    ...
    if (custom_text)
        ...
        else for (var i in custom_text) ta[i] = custom_text[i];
    ...
    var style =
        "<style>.buorg-icon {width: 22px; height: 16px; vertical-align: middle; position: relative; top: -0.05em; display: inline-block; background: no-repeat 0px center url(https://browser-update.org/static/img/small/" +
        bb.n +
        ".png);}</style>";
    var style2 =
        "<style>.buorg {position:absolute;position:fixed;z-index:111111; width:100%; bottom:0px; left:0px; text-align:center;  color:#000; background-color: #fff8ea; font: 18px Calibri,Helvetica,sans-serif; box-shadow: 0 0 5px rgba(0,0,0,0.2);animation: buorgfly 1s ease-out 0s;}" +
    ...
    if (!ta.msg && t.indexOf && t.indexOf("%s") !== -1) {
        ...
        style +=
            "<style>.buorg {position:absolute;position:fixed;z-index:111111; width:100%; top:0px; left:0px; text-align:left; cursor:pointer; color:#000; font: 13px Arial,sans-serif;color:#000;}" +
        ...
        if (op.style === "bottom") style2 += "<style>.buorg { font-size: 12px; font-family: Roboto, \"Helvetica Neue\", sans-serif; bottom:0; top:auto; } @keyframes buorgfly {from {opacity:0;transform:translateY(50px)} to {opacity:1;transform:translateY(0px)}}</style>";
        else if (op.style === "corner")
            style2 +=
                "<style> .buorg {border-radius: 10px 10px; text-align: left; width:370px; height: 130px; bottom:50px; right:50px; left:auto; } .buorg-mainmsg, .buorg-moremsg { display: block; } .buorg-buttons a {margin: 4px 2px;} .buorg-icon {display: inline-block;} @media(max-width: 500px){.buorg {border-radius: 10px 10px; text-align: left; width:300px; height: 130px; bottom:50px; right:50px; left:auto; } .buorg-mainmsg, .buorg-moremsg { display: block; } .buorg-buttons a {margin: 4px 2px;} .buorg-icon {display: inline-block;}}</style>";
        else op.addmargin = true;
        t = t
            .replace("{brow_name}", bb.t)
            .replace("{up_but}", ' id="buorgul" href="' + op.url + '"' + tar)
            .replace("{ignore_but}", ' id="buorgig" role="button" tabindex="0"');
        ...
        ignorebutton.onclick = function (e) {
            ...
            if (!op.no_permanent_hide && ta.bnever && ta.remind) {
                op.div.innerHTML =
                    ...
                    (op.reminderClosed > 24 ? ta.remind.replace("{days}", Math.round(op.reminderClosed / 24)) : "") +
                    '</span> <span class="buorg-buttons"><a id="buorgpermanent" role="button" tabindex="0" href="' +
