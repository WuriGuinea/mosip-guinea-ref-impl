...
var $buo_show = function () {
    ...
    var style =
        ...
        ".png);}</style>";
    var style2 =
        "<style>.buorg {position:absolute;position:fixed;z-index:111111; width:100%; bottom:0px; left:0px; border-bottom:1px solid #A29330; text-align:center;  color:#000; background-color: #fff8ea; font: 18px Calibri,Helvetica,sans-serif; box-shadow: 0 0 5px rgba(0,0,0,0.2);animation: buorgfly 1s ease-out 0s;}" +
    ...
    if (!ta.msg && t.indexOf && t.indexOf("%s") !== -1) {
        ...
        if (op.style === "bottom") style2 += "<style>.buorg {bottom:0; top:auto; border-top:1px solid #A29330; } @keyframes buorgfly {from {opacity:0;transform:translateY(50px)} to {opacity:1;transform:translateY(0px)}}</style>";
            ...
            style2 +=
                "<style> .buorg { text-align: left; width:300px; bottom:50px; right:50px; left:auto; border:1px solid #A29330; } .buorg-buttons, .buorg-mainmsg, .buorg-moremsg { display: block; } .buorg-buttons a {margin: 4px 2px;} .buorg-icon {display: none;}</style>";
